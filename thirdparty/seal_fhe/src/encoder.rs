...
pub struct BFVEncoder<'a> {
    handle: *mut c_void,
    params: &'a EncryptionParameters,
}

unsafe impl<'a> Sync for BFVEncoder<'a> {}
unsafe impl<'a> Send for BFVEncoder<'a> {}
...
impl<'a> BFVEncoder<'a> {
     ...
     * * `ctx` - The Context
     */
    pub fn new(ctx: &Context, params: &'a EncryptionParameters) -> Result<Self> {
        let mut handle: *mut c_void = null_mut();
        let err = unsafe { bindgen::BatchEncoder_Create(ctx.get_handle(), &mut handle) };

        convert_seal_error(err)?;
    ...
    pub fn get_params(&self) -> &EncryptionParameters {
        self.params
    }

    /**
     * Creates a random plaintext element
     */
    pub fn random_pt(&self) -> Result<Plaintext> {
        let plain_modulus = self.params.get_plain_modulus().value();
        let random_u64 = rand::random_range(0..plain_modulus);
        self.encode_unsigned(&[random_u64])
    }
