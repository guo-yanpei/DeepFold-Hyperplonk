...
namespace seal
{
    BatchEncoder::BatchEncoder(const SEALContext &context) : context_(context)
    {
        ...
        {
            std::cout << "error: encryption parameters are not set correctly" << endl;
        ...
        {
            std::cout << "error: unsupported scheme" << endl;
            throw invalid_argument("unsupported scheme");
        }
        if (!context_data.qualifiers().using_batching)
        {
            std::cout << "error: encryption parameters are not valid for batching" << endl;
